...
// Cualquier usuario registrado en BD puede enviar un formulario
// =========================
app.post('/api/contact-submissions', requireAuth, async (req, res) => {
...
// SOLO coordinador autorizado
// =========================
app.get('/api/contact-submissions', requireAuth, async (req, res) => {
  if (!isCoordinator(req)) {
    return res.status(403).json({ error: 'Acceso denegado: Solo coordinadores' });
  }
...
// =========================
// CONTACT SUBMISSION UPDATE STATUS
// Para marcar como 'contacted' o 'archived'
// =========================
app.patch('/api/contact-submissions/:id', requireAuth, async (req, res) => {
  if (!isCoordinator(req)) {
    return res.status(403).json({ error: 'Solo coordinadores' });
  }

  const { status } = req.body; // Se espera: 'pending', 'contacted', 'archived'
